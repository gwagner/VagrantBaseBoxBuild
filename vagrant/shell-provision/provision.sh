...
# Install git so we can get all the proper puppet modules from github
    ...
    if [ ! -f /usr/bin/git ]
    then
        echo "Installing Git to download Puppet Modules to provision VM"
...
# Finally checkout all of the modules we need to build the machine
    ...
    # modules[mongo-server]="https://github.com/gwagner/puppet-mongo-server.git"
    ...
    modules[repo_10gen]="https://github.com/gwagner/puppet-repo-10gen.git"
    ...
    do
        ...
        then
            ...
            echo "$mod up to date and ready to use"
        fi
    done
